  ...
  return async (dispatch) => {
    try {
      ...
      dispatch({
        ...
        payload: { signUpSuccessMessage: res.data.result },
      });
    } catch (error) {
      dispatch({
...
export const activeAccount = (userId, activeCode) => {
  return async (dispatch) => {
    try {
      dispatch({ type: authConstants.ACTIVE_ACCOUNT_REQUEST });
      const response = await axios.get(`/active/${userId}/${activeCode}`);
      dispatch({
        type: authConstants.ACTIVE_ACCOUNT_SUCCESS,
        payload: {
          activeSuccessMessage: response.data.data,
        },
      });
    } catch (error) {
      dispatch({
        type: authConstants.ACTIVE_ACCOUNT_FAILURE,
        payload: { activeErrorMessage: error.response.data.error },
      });
    }
  };
};
